...
export class GroupDetailsComponent implements OnInit {
  ...
  private userPhone = this.userService.user.telephone;
  ...
  private getUserPot() {
    this.backendService.getUserPot(this.userPhone, this.selectedGroup.potId)
      .subscribe(
        data => {
  ...
  leaveGroup() {
    ...
    this.groupService.leavePot(potId).subscribe(
      (confirmed)=>{
        ...
        this.userService.updateUserGroups(this.userPhone);
        this.userService.updateUserInLocalStorage(this.userPhone);
  ...
  borrowMoney(amount: string) {
    ...
    let userId = `${this.userPhone}`;
    let potId = `${this.selectedGroup.potId}`;
    ...
    this.groupService.getLoans(this.selectedGroup.potId).subscribe(
      data => {
        ...
        this._loans = loans
          .filter(
            (loan) =>
              loan.user.telephone === this.userPhone &&
    ...
    this.userService.getMemberList(this.selectedGroup.potId).subscribe(
      data => {
        this._members = data.json().filter(
          (user: User) => user.telephone !== this.userPhone
        );
      }
    );
  }
