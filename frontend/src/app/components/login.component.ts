...
export class Login implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.loginService.isLoggedIn()) {
      this.loading = true;
      this.setUser(parseInt(localStorage.getItem('telephone')));
    }
  }
  ...
  initiateTokenHandshake(loginFormModel: {'telephone':string, 'password':string}) {
    ...
    this.loginService.getTokenForUser(loginPhone, loginPass).subscribe(
      data => {
        ...
        this.loading = true;
        this.setUser(parseInt(loginPhone));
      },
      error => {
  ...
  setUser(phoneNumber: number) {
    this.userService.getUser(phoneNumber).subscribe(
      response => {
        ...
        this.setGroupsForUser(phoneNumber);
      },
      error => {
  ...
  setGroupsForUser(userPhone: number) {
    this.userService.getGroups(userPhone).subscribe(
      data => {
